...
export default new Command({
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const msg = await interaction.editReply({ components: [container, buttons], flags: MessageFlags.IsComponentsV2 as any });
    ...
    collector.on('collect', async (i) => {
      if (i.customId.startsWith('prestige_confirm')) {
        ...
        await i.update({ components: [result], flags: MessageFlags.IsComponentsV2 as any });
      } else {
